#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════
#  build_linux.sh  —  Builds pi-dashboard-linux.deb
...
#
#  Output:  dist/pi-dashboard-linux.deb
# ═══════════════════════════════════════════════════════════════════════════
set -e

VERSION="10.0"
ARCH=$(dpkg --print-architecture 2>/dev/null || uname -m | sed 's/x86_64/amd64/;s/aarch64/arm64/')
PKG_NAME="pi-dashboard"
DEB_NAME="${PKG_NAME}-linux.deb"

echo
echo "───────────────────────────────────────────────────────────"
echo "  Pi Dashboard — Linux .deb Build"
...
rm -f  "dist/${DEB_NAME}" "dist/pi-dashboard"

# ── 4. PyInstaller ───────────────────────────────────────────
echo "  Running PyInstaller…"
pyinstaller pi-dashboard.spec --noconfirm

if [ ! -f "dist/pi-dashboard" ]; then
    echo "  ERROR: dist/pi-dashboard not found after PyInstaller run."
    exit 1
fi
...
# Copy the binary
cp "dist/pi-dashboard" "${BIN_DIR}/pi-dashboard"
chmod 755 "${BIN_DIR}/pi-dashboard"

# Minimal README
cat > "${DOC_DIR}/README" <<'DOC'
Pi Dashboard v10.0 — cross-platform system monitor.
Run: pi-dashboard
First run opens the setup wizard in your browser.
Docs: https://YOUR_USERNAME.github.io/pi-dashboard
DOC
...
Description: Pi Dashboard — cross-platform system monitor
...
Homepage: https://YOUR_USERNAME.github.io/pi-dashboard
CTRL
...
set -e
echo ""
echo "  Pi Dashboard installed to /usr/local/bin/pi-dashboard"
echo "  Run:  pi-dashboard"
...
systemctl --user stop   pi-dashboard 2>/dev/null || true
systemctl --user disable pi-dashboard 2>/dev/null || true
PRERM
